                    ...
                    switch (currentMode) {
                        ...
                        case MODE_CAMERA_MANIPULATION:
                            if (cursorContainer.getNumCursors() == 2 && type == MotionEvent.ACTION_MOVE) {
                                ...
                                //We get the points where the dragging starts and where it stops
                        ...
                        case MODE_SHAPE_MANIPULATION:
                            if (cursorContainer.getNumCursors() == 2 && type == MotionEvent.ACTION_MOVE && indexOfShapeBeingManipulated >= 0) {
                                ...
                                //We check if there is already a shape being manipulated
                                if (indexOfShapeBeingManipulated != -1) {
                                    indexSelectedShape = indexOfShapeBeingManipulated;
                                }
                                else {
                                    //We check if at the finger position there is a shape
                                    indexSelectedShape = shapeContainer.indexOfShapeContainingGivenPoint(cursor0.getCurrentPosition());
                                }

                                //If a shape has been found
                                if (indexSelectedShape > -1) {

                                    //We set the index of the current shape selected
                                    indexOfShapeBeingManipulated = indexSelectedShape;

                                    //We get the shape from the shape container with it's index
                                    ...
                                    //We update the position of the points of the shape
                                    selectedShape.applyTranslation(translation);

                                    //We redraw the shape with the translation applyed
                                    selectedShape.draw(gw, true);
